...
export class OAuth2AuthCodePKCE {
    ...
    public isAccessTokenExpired(): boolean {
        const authInfo = JSON.parse(localStorage.getItem(LOCALSTORAGE_STATE));
        if (localStorage.hasOwnProperty(LOCALSTORAGE_STATE)
            && authInfo.accessToken
            && authInfo.accessToken.value
            && authInfo.accessToken.expiry) {

            const { accessToken } = this.state;

            return Boolean((new Date()) >= (new Date(accessToken.expiry)));
        }
        return true;       
    }
